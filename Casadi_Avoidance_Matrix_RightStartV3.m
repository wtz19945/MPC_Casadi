...
avd_vel = MX.sym('avd_vel',4);

Variable = [x;ux;dPx;y;uy;dPy;s;swf_q;swf_s;avd_vel;swf_bi];
...
swf_obs = MX.sym('swf_obs', 14);    % Obstacle x,y,z, radius, weights, z fraction
...
x_vel = x(2:2:end);
y_vel = y(2:2:end);
dx_e = avd_vel(1) * ones(4,1);
ieq_con = [ieq_con;x_vel(round(Tstep / dt)+1 - step_index:round(Tstep / dt):end) - x_ref(2:5) - avd_vel(1) - avd_vel(2)];
ieq_con = [ieq_con;-x_vel(round(Tstep / dt)+1 - step_index:round(Tstep / dt):end) + x_ref(2:5) - avd_vel(1) - avd_vel(3)];
...
if step_index < 3
    for i = 2:5-step_index
        ...
        dx_e = [dx_e;avd_vel(1)];
        ieq_con = [ieq_con;x_vel(i) - x_ref(2) - avd_vel(1) - avd_vel(2);-x_vel(i) + x_ref(2) - avd_vel(1) - avd_vel(3)];
    end
...
% Binary constraints on avodance switch
ieq_con = [ieq_con;-avd_vel(1:3);swf_s(5) - avd_vel(4) * M;avd_vel(2);avd_vel(3)];
cost = cost + swf_obs(14) * avd_vel(4) * avd_vel(4);

...
q_init = [8.86;-0.01;-0.08;0.07]; % intial robot state
q_ref = [0;0.3;0;0.0];  % robot state ref
f_length = [.8;.4];     % foot length limit
f_init = [8.84;-0.16];      % foot initial state
f_param = [0;0;.3 * .4;0;0;0.14]; % foot parameters
Weights = [0;3000;0;1000;10000;10000;100;3500;0]; % MPC weights
r = [0.2;0.4];          % obstacle radius
qo_ic = [-10.4;0.0];      % obstacle position
qo_tan = [1;0]; % obstacle tangent vector
qo_tan = qo_tan/norm(qo_tan);

Input = [q_init;q_ref;f_length;f_init;f_param;Weights;r;qo_ic;qo_tan;0.1;0;0];

[a,b,c,d,e,f] = RightStart_Step3(Input,0*rand(144,1));

[a1,b1,c1,d1,e1,f1] = RightStart_Step3(Input,2.5*rand(144,1));

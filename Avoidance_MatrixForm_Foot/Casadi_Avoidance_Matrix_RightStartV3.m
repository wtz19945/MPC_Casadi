...
swf_obs = MX.sym('swf_obs', 7);    % Obstacle x,y,z, radius, weights, z fraction
...
for n = step_index + 1 : round(Tstep / dt) + 1
    ...
    if n == step_index + 1
        ...
        inter = swf_obs_pos + swf_obs_r2 * vec;
        inter1 = swf_obs_pos + swf_obs_r1 * vec;
        ieq_con = [ieq_con;-dot(cur_p - inter,vec) - swf_s(n)];
        cost = cost + swf_obs_Q * swf_s(n) * swf_s(n);
    end
end

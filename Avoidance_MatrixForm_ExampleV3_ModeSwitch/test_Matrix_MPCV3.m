...
q_init = [0;0.1;0;-0.06]; % intial robot state
dx_des = 0.3;
w = sqrt(9.81 / 0.9);
...
f_init = [0.0;-0.11];      % foot initial state
f_param = [0;0;x0_ref;0.0;0;0.11]; % foot parameters
Weights = [0;5000;0;5000;10000;10000;500;500;5000]; % MPC weights
r = [0.2;0.4];          % obstacle radius
qo_ic = [.8;0.1];       % obstacle position
...
[a,b,c,d,e,f] = RightStart_Step0V3(Input,0*rand(135,1));

[a1,b1,c1,d1,e1,f1] = RightStart_Step0V3(Input,2.5*rand(135,1));
...
ux = design_vector(Nodes * 2 + 1: Nodes * 3);
uy = design_vector(5*Nodes + Npred + 1:6*Nodes + Npred);

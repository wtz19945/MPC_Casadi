function [] = Casadi_Avoidance_QP_LeftStart(step_index)
...
tic
f_MPC_opti = MPC_opti.to_function(char('LeftStartQP_Step' + string(step_index)),{x,ux,dPx,y,uy,dPy,s,s2,q_init,q_ref,f_length,f_init,f_param,...
    Weights,r,qo_ic,qo_tan},{x,ux,dPx,y,uy,dPy,s,s2});
toc

%cg_options = struct('mex', true,'cpp',true,'main',true,'with_header',true);
cg_options = struct();
cg = CodeGenerator(char('LeftStartQP_Step' + string(step_index)),cg_options);
cg.add(f_MPC_opti);

function [] = Casadi_Avoidance_Matrix_LeftStart(step_index)
import casadi.*
% System Parameters
...
foff = MX.sym('foff', 1);
Input = [q_init;q_ref;f_length;f_init;f_param;Weights;r;qo_ic;qo_tan;rt];
...
fff = Function(char('LeftStart_Step' + string(step_index)),{Input,Variable},{Aeq,beq,Aiq,biq,H,f});
% opts = struct('mex',false,'main',false,'cpp',false,'with_header',false);
% func_name = char('LeftStart_Step' + string(step_index));
% fff.generate(func_name,opts);

cg_options = struct('mex',true,'main',false,'cpp',false,'with_header',false);
cg = CodeGenerator(char('LeftStart_Step' + string(step_index)),cg_options);
cg.add(fff);
tic
cg.generate() 
toc

function [] = Casadi_Avoidance_Matrix_RightStart(step_index)
import casadi.*
% System Parameters
...
foff = MX.sym('foff', 1);
Input = [q_init;q_ref;f_length;f_init;f_param;Weights;r;qo_ic;qo_tan;rt;foff];

% Define Dynamics Constraint
eq_con = [];
for n = 1:Nodes
    %
    ux_n = ux(nu*(n-1)+1:nu*n) + foff;
    x_n = x(nx*(n-1)+1:nx*n);
    ...
    if n == 1
        ...
        if n == 2
            eq_con = [eq_con;x_n - x_n1 - rt * (A*x_n + B*ux_n);y_n - y_n1 - rt * (A*y_n + B*uy_n)];
        else
            eq_con = [eq_con;x_n - x_n1 - dt * (A*x_n + B*ux_n);y_n - y_n1 - dt * (A*y_n + B*uy_n)];
        end
    end
end
...
disp('generating c c')
fff = Function(char('RightStart_Step' + string(step_index)),{Input,Variable},{Aeq,beq,Aiq,biq,H,f});
...
cg = CodeGenerator(char('RightStart_Step' + string(step_index)),cg_options);
cg.add(fff);
tic
cg.generate() 
toc
...
% qo_ic = [1.8;0.1];
% qo_tan = [];
% Input = [q_init;qo_ic;qo_tan;q_ref;f_length;f_init;f_param;Weights;r];
% 
